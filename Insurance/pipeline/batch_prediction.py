...
def start_batch_prediction(input_file_path):
    try:
        ...
        log.info(f"Creating model resolver object")
        model_resolver = ModelResolver(model_registry="saved_models")
        log.info(f"Reading file :{input_file_path}")
        df = pd.read_csv(input_file_path)
        df.replace({"na": np.NAN}, inplace=True)
        # validation

        log.info(f"Loading transformer to transform dataset")
        transformer = load_object(
            file_path=model_resolver.get_latest_transformer_path())

       
        log.info(f"Target encoder to convert predicted column into categorical")
        ...
        log.info(f"Loading model to make prediction")
