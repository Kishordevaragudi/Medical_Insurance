...
class ModelEvaluation:
    ...
    def __init__(self,
        ...
        try:
            log.info(f"{'>>'*20}  Model Evaluation {'<<'*20}")
    ...
    def initiate_model_evaluation(self)->artifact_entity.ModelEvaluationArtifact:
        try:
            ...
            log.info("if saved model folder has model the we will compare "
            "which model is best trained or the model from saved model folder")
            latest_dir_path = self.model_resolver.get_latest_dir_path()
            if latest_dir_path==None:
                model_eval_artifact = artifact_entity.ModelEvaluationArtifact(is_model_accepted=True,
                improved_accuracy=None)
                log.info(f"Model evaluation artifact: {model_eval_artifact}")

                return model_eval_artifact


            #Finding location of transformer model and target encoder
            log.info("Finding location of transformer model and target encoder")
            transformer_path = self.model_resolver.get_latest_transformer_path()
            model_path = self.model_resolver.get_latest_model_path()
            target_encoder_path = self.model_resolver.get_latest_target_encoder_path()

            log.info("Previous trained objects of transformer, model and target encoder")
            ...
            log.info("Currently trained model objects")
            ...
            log.info(f"Accuracy using previous trained model: {previous_model_score}")
            ...
            log.info(f"Accuracy using current trained model: {current_model_score}")
            if current_model_score<=previous_model_score:
                log.info(f"Current trained model is not better than previous model")
                raise Exception("Current trained model is not better than previous model")

            
            model_eval_artifact = artifact_entity.ModelEvaluationArtifact(is_model_accepted=True,
            improved_accuracy=current_model_score-previous_model_score)
            log.info(f"Model eval artifact: {model_eval_artifact}")
            return model_eval_artifact
