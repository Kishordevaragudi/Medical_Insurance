...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame, report_key_name:str)->Optional[pd.DataFrame]:
        ...
        try:
            ...
            log.info(f"selecting column name which contains null above to {threshold}")
            drop_columns_names = null_report[null_report > threshold].index
            
            log.info(f"Columns to drop:{list(drop_columns_names)}")
        ...
        try:
            ...
            for base_column in base_columns:
                if base_column not in current_columns:
                    log.info(f"column: [{base} is not avaliable]")
    ...
    def initaite_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            log.info(f"Reading base dataframe")
            base_df = pd.read_csv(self.data_validation_config.base_file_path)
            base_df.replace({"na":np.NAN},inplace=True)
            log.info(f"Replace na value in base df")
            #base_df has na as null
            log.info(f"Drop null values columns from base df")
            base_df=self.drop_missing_values_columns(df=base_df,report_key_name="missing_values_within_base_dataset")

            log.info(f"Reading train dataframe")
            train_df = pd.read_csv(self.data_ingestion_artifact.train_file_path)
            log.info(f"Reading test dataframe")
            ...
            log.info(f"Is all required columns present in train df")
            ...
            log.info(f"Is all required columns present in test df")
            ...
            if train_df_columns_status:
                log.info(f"As all column are available in train df hence detecting data drift")
                self.data_drift(base_df=base_df, current_df=train_df,report_key_name="data_drift_within_train_dataset")
            if test_df_columns_status:
                log.info(f"As all column are available in test df hence detecting data drift")
                self.data_drift(base_df=base_df, current_df=test_df,report_key_name="data_drift_within_test_dataset")
          
            #write the report
            log.info("Write reprt in yaml file")
