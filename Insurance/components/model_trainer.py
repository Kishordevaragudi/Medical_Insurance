...
class ModelTrainer:
    ...
    def train_model(self, x, y):
        try:
            log.info(f"Initializing the linearRegression model")
            lr = LinearRegression()
            lr.fit(x, y)
            return lr
        except Exception as e:
    ...
    def initiate_model_trainer(self,)->artifact_entity.ModelTrainerArtifact:
        try:
            log.info(f"load the data from the utils.py")
            train_arr = utils.load_numpy_array_data(file_path=self.data_transformation_artifact.transformed_train_path)
            test_arr = utils.load_numpy_array_data(file_path=self.data_transformation_artifact.transformed_test_path)

            log.info(f"spliting the data")
            ...
            log.info(f"prediction of the model and checking the accuracy score")
            ...
            if r2_test_score < self.model_trainer_config.expected_accuracy:
                ...
                log.info(f"Model is not good as it is not able to give \
                expected accuracy:{self.model_trainer_config.expected_accuracy}: model acutal score: {r2_test_score}")
            ...
            if diff > self.model_trainer_config.overfitting_threshold:
                ...
                log.info((f"Train and test score diff: {diff} is more than overfitting threshold {self.model_trainer_config.overfitting_threshold}"))
